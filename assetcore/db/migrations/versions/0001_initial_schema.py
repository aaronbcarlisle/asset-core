...
Portable SQLAlchemy types so this runs on both Postgres (prod) and sqlite (CI
verification). Mirrors infra/schema.sql field-for-field with the tool-agnostic
column names. UUIDs are stored as strings for portability; the repos already
serialize UUID<->str at the boundary.
"""
...
def upgrade() -> None:
    ...
    op.create_table(
        "facet_identity",
        sa.Column("asset_id", sa.String(), sa.ForeignKey("asset.id", ondelete="CASCADE"),
                  primary_key=True),
    ...
    op.create_table(
        "relationship",
        sa.Column("id", sa.BigInteger(), primary_key=True, autoincrement=True),
        sa.Column("from_asset", sa.String(), sa.ForeignKey("asset.id", ondelete="CASCADE"),
                  nullable=False),
        sa.Column("to_asset", sa.String(), sa.ForeignKey("asset.id", ondelete="CASCADE"),
                  nullable=False),
    ...
    op.create_table(
        "event",
        sa.Column("id", sa.BigInteger(), primary_key=True, autoincrement=True),
        sa.Column("asset_id", sa.String(), sa.ForeignKey("asset.id", ondelete="SET NULL")),
