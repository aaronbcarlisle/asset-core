"""The three scenarios + invariants, ported from tests/test_scenarios.py to run
against InMemoryRepo — no database, milliseconds.

Same production pain points, same verbs, proven through the pure core. Deltas from
the SQLite original (see PHASE1 plan): facets come back as entity objects (attr
access, location_uri not depot_path); resolve_dependency returns a SourceVersion
(mode is read off the edge); the Scenario-3 pin uses set_binding (relate asserts a
new edge, set_binding flips an existing one). Plus a new event-spine assertion now
that the sink is inspectable.
"""
import pytest
...
# ---------------------------------------------------------------------------
def test_barrel_reuse_and_lineage(repo, sink):
...
# ---------------------------------------------------------------------------
def test_robin_locomotion_semantics(repo, sink):
...
# ---------------------------------------------------------------------------
def test_materials_float_then_pin(repo, sink):
...
# ---------------------------------------------------------------------------
def test_resolve_all_three_facets(repo, sink):
...
# ---------------------------------------------------------------------------
def test_rename_does_not_touch_source_or_runtime(repo, sink):
    a = verbs.declare(repo, sink, "prop", "env_amy")
    verbs.bind_source(repo, sink, a, "//depot/props/barrel.ma", "maya", 4101, "env_amy")
...
# ---------------------------------------------------------------------------
def test_single_latest_source_version(repo, sink):
    a = verbs.declare(repo, sink, "prop", "env_amy")
...
# ---------------------------------------------------------------------------
# Event spine — the sink records the facet writes, in order
# ---------------------------------------------------------------------------
def test_event_spine_records_declares_and_writes(repo, sink):
    a = verbs.declare(repo, sink, "prop", "env_amy")
    verbs.bind_source(repo, sink, a, "//depot/props/barrel.ma", "maya", 4101, "env_amy")
    verbs.claim(repo, sink, a, "Barrel", "props/barrel", "pat")
