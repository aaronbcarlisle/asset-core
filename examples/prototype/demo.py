"""
demo.py — the ORIGINAL prototype walkthrough (frozen reference).

Run:  python examples/prototype/demo.py

This drives the prototype seed (`api.py` over a backend-agnostic `SqliteDB`) — the
single-file design that predates the layered `assetcore` package. It is kept as
reference for how the model started; the maintained, layered demo is the repo-root
`demo.py`. See this folder's README.
"""
import pathlib
import sys

sys.path.insert(0, str(pathlib.Path(__file__).parent))   # run self-contained

import api                                    # noqa: E402  (the prototype API)
from connection import SqliteDB               # noqa: E402
